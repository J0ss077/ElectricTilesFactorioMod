...
function module.make_electric_variant(collection)
    --
    for i1, base_data in ipairs(collection) do
        --
        for i2, obj in ipairs(base_data) do
            --
            if type(obj) ~= "table" then error("all data must be tables !!") end
        end
        ...
        --------------------------------------------------------------------------
        table.insert(carrier.data["allowed-tiles"] --[[@as table]], new_tile.name)
        --------------------------------------------------------------------------

        if not base_data.item then goto F077ET_continue_001 end
        ...
        if not (base_data.recipe or others.use_default_recipe) then goto F077ET_continue_001 end
